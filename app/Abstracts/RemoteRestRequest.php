...
class RemoteRestRequest
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            } else if ($this->type == 'nosoap') {

                $headers = $requestObjectForLog['headers'];
                $auth = $requestObjectForLog['auth'];

                $client = new nusoap_client($this->base_uri, 'wsdl');
                $err = $client->getError();
                if ($err) {
                    echo '<h2>nusoap hatası</h2>' . $err;
                    exit();
                }

                $params = $this->objToArray(json_decode($this->body, true));
                $result = $client->call('addshipment', $params);

                $this->setContent($result);
                // $this->setStatusCode($this->soapStatusCode($client));
            } else {
