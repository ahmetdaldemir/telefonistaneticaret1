...
$(document).ready(function() {
    ...
    var product_variant_id = $('input[name="product_variant_id"]').val();
    ...
    var $fileuploader = $('input.gallery_media').fileuploader({
        ...
        upload: {
            url: './edit-image-upload?type=upload&product_id='+product_id+'&product_variant_id='+product_variant_id+'',
            data: null,
            type: 'POST',
        ...
        editor: {
            ...
            onSave: function(dataURL, item) {
                ...
                if (item.appended && item.data.listProps) {
                    ...
                    $.post('/product/edit-image-upload?type=resize', {name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
                        // update the image
        ...
        sorter: {
            onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
                ...
                // send request
                $.post('/product/edit-image-upload?type=sort', {
            ...
            $('body').on('click', function(e) {
                ...
                if ($target.is('.gallery-action-rename')) {
                    ...
                    if (x && item.data.listProps) {
                        $.post('/product/edit-image-upload?type=rename', {name: item.name, id: item.data.listProps.id, title: x}, function(result) {
                            try {
                ...
                if ($target.is('.gallery-action-asmain') && item.data.listProps) {
                    $.post('/product/edit-image-upload?type=asmain', {name: item.name, id: item.data.listProps.id}, function() {
        ...
        onRemove: function(item) {
            // send request
            if (item.data.listProps)
                $.post('/product/edit-image-upload?type=remove', {
                    name: item.name,
                    id: item.data.listProps.id
                });
        },
    ...
    // preload the files
    $.post('/product/edit-image-upload?type=preload&id='+product_id+'&product_variant_id='+product_variant_id+'', null, function(result) {
