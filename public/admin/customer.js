...
$(function () {
    ...
    $('#datatable').on('click', '.status-change', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '/customer/statusupdate?id=' + id + '&status='+status+'',
                    type: 'PUT',
    ...
    $('#datatable').on('click', '.edit-button', function () {
        ...
        $('#customerBasic').modal('show');
        $('#customerBasic #customerForm').find('input#id').val(data.id);
        $('#customerBasic #customerForm').find('input#name').val(data.names);
        $('#customerBasic #customerForm select#parent_id').val(data.parent?data.parent.id:0).trigger('change');
