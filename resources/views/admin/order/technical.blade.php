...
@section('content')
    <div class="modal fade" id="technicalBasic" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog  dialog  md:max-w-[1100px]">
            <div class="dialog-content">
                ...
                <form method="post" action="{{route('order.technicalUpdate')}}" id="technicalUpdateForm" enctype="multipart/form-data">
                    @csrf
    ...
    <main class="h-full">
        <div class="page-container relative h-full flex flex-auto flex-col px-4 sm:px-6 md:px-8 py-4 sm:py-6">
            <div class="card adaptable-card">
                <div class="card-body">
                    <h3 class="mb-4">Teknik Siparişler</h3>
                        ...
                        <table id="order-list-table" class="table-default table-hover">
                            ...
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td>
                                        <select class="input input-sm" data-id="{{$order->id}}" id="statusSelect">
                                    ...
                                    <td class="text-right">
                                        <span class="text-red-600">{{$order->proforma_total_price}} ₺</span>
                                    </td>
                                    <td class="text-right">
                                        <span class="text-red-600">{{$order->reel_total_price}} ₺</span>
                                    </td>
                                    <td class="text-right">
                                        <span class=" btn btn-two-tune btn-sm btn-icon rounded-full" data-bs-toggle="tooltip" data-bs-html="true" data-bs-title="{!! $order->description !!} <br>{!! $order->description_price !!} ">
                                    ...
                                    <td>
                                        <div class="flex justify-end text-lg">
                                            @if($order->reel_total_price == 0 || $order->status == 0)
                                              <button data-id="{{$order->id}}" data-bs-toggle="modal" data-bs-target="#technicalBasic" id="priceUpdate" class="btn  btn-xs bg-orange-500 hover:bg-orange-400 active:bg-orange-600 text-white">Fiyat Belirle</button>
                                            @endif
                                            <a href="{{route('order.technicalOrder_detail',['id' => $order->id])}}" class="cursor-pointer p-2 hover:text-indigo-600" data-bs-toggle="tooltip" data-bs-title="Goruntule">
